...
- [ ] New `McpPage.tsx`: list / install-from-raw-TOML / view (plaintext) / edit / delete-with-confirmation
- [ ] Preset detail page tab bar: "技能库" (existing) | "MCP库" (new) with counts; MCP tab lists servers with add/remove controls and name+command excerpts
- [ ] PresetBar `handleActivate` → unified `POST /api/presets/:ref/apply`; `handleDeactivate` → unified `POST /api/presets/deactivate` (internal, no new optional prop)
- [ ] Project workspace PresetBar non-blocking notice "MCP 配置全局生效，不限于当前项目" on activation
- [ ] Settings tool card: "MCP 输出目录" input + browse; "MCP 输出格式" dropdown (TOML/JSON, JSON hidden for tools whose format list excludes it, e.g. codex) → `PUT /api/tools/:key/mcp`
- [ ] Sidebar "MCP库" `NAV_ITEMS` entry below "技能库"; preset labels keep skill-only counts

## Acceptance criteria

- [ ] Sidebar renders a "MCP库" entry below "技能库"; navigating to it renders `McpPage`
- [ ] `McpPage` lists installed MCP servers; install via raw TOML content parses the server name and creates the server; view shows full plaintext TOML; edit pre-fills the current full content and updates in-place without changing the server name; delete prompts confirmation and removes the server
- [ ] Preset detail page renders a tab bar with counts ("技能库 {N}" / "MCP库 {M}"); "技能库" preserves existing skill-list behavior; "MCP库" lists the preset's MCP servers with server-name + command excerpts (no env) and add/remove controls that call the membership routes
- [ ] PresetBar activation triggers `POST /api/presets/:ref/apply` and deactivation triggers `POST /api/presets/deactivate`; MCP is synced as part of the same operation (no separate MCP button/prop)
- [ ] Project workspace PresetBar surfaces the "MCP 配置全局生效，不限于当前项目" notice on activation; global workspace PresetBar does not (MCP is already global there)
- [ ] Settings tool card shows "MCP 输出目录" + "MCP 输出格式"; the JSON option is hidden for tools whose supported formats exclude it (e.g. codex); saving calls `PUT /api/tools/:key/mcp`
- [ ] Sidebar preset labels show skill counts only; MCP counts appear only in the preset detail tabs
- [ ] Client builds without type/lint regressions: `npm --workspace web/client run build` and lint pass

## Suggested verification

- [ ] `npm --workspace web/client run build` and lint
