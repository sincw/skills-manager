...
mod tests {
    ...
    fn clone_into_unlocked_failure_redacts_credentials_in_url() {
        let tmp = tempfile::tempdir().unwrap();
        let target = tmp.path().join("clone-target");
        // Malformed token-bearing URL that fails locally without network I/O.
        // The wrapper must not include the raw input URL in the error.
        let url = "https://ghp_supersecrettoken123@/does-not-exist.git";
    ...
    fn pull_conflict_aborts_merge_and_reports_sync_conflict() {
        ...
        assert!(Command::new("git")
            .args(["clone", "--branch", "main", remote.to_str().unwrap()])
            .arg(&b)
            .output()
            .unwrap()
            .status
