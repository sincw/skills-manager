...
export function AddProjectDialog({ open, onClose, onAdded }: Props) {
  ...
  const handleAddManualPath = async () => {
    ...
    setAdding(true);
    try {
      await api.addProject(manualPath.trim());
      await onAdded();
      onClose();
    } catch {
      // error handled by toast in parent
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={adding}
                className="px-2.5 rounded-[4px] border border-border-subtle bg-background text-muted hover:text-secondary hover:border-border transition-all outline-none disabled:opacity-50"
                title={t("project.addManual")}
              >
